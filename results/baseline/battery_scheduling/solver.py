...
class Solver:
    ...
    def solve(self, problem: dict) -> dict:
        ...
        try:
            ...
            return {'status': prob.status, 'optimal': True, 'battery_results': [{'q': q.value.tolist(), 'c': c_net.tolist(), 'c_in': c_in.value.tolist(), 'c_out': c_out.value.tolist(), 'cost': cost_with_battery}], 'total_charging': c_net.tolist(), 'cost_without_battery': cost_without_battery, 'cost_with_battery': cost_with_battery, 'savings': savings, 'savings_percent': float(100 * savings / cost_without_battery)}
